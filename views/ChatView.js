...
const ChatView = {
  setup() {
    ...
    function send() {
      ...
      // ユーザーメッセージ追加
      store.messages.push({ who: 'user', text: input.value, avatar: 'https://i.pravatar.cc/40?img=3' });
  ...
  template: `
    <div class="container py-4">
      ...
      <div id="chatScroll" class="chat-body mb-3 position-relative">
        <div v-for="(m,i) in store.messages" :key="i" class="d-flex mb-2"
             :class="m.who==='user' ? 'justify-content-end' : 'justify-content-start'">
          <img :src="m.avatar" class="rounded-circle me-2" width="40" height="40" />
          <div class="msg p-2" :class="m.who==='user' ? 'user' : 'ai'">
            {{ m.text }}
          </div>
        </div>
        <div v-if="isTyping" class="d-flex align-items-center">
          <img src="https://i.pravatar.cc/40?img=12" class="rounded-circle me-2" width="40" height="40" />
      ...
      <div class="mb-3">
        ...
        <button v-for="(q,i) in store.quickReplies" :key="i"
